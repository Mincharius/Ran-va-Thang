...
export default class Server {
  constructor() {
    ...
    this.isOnJoinSuccess = false;
    this.isOnJoinError = false;
  }
  ...
  async joinById(roomId) {
    ...
    room.onMessage("roomId", async (state) => {
      if(state){
        ...
        this.events.emit('join-success')
      }else{
        this.events.emit('join-error')
      }
     
    });
  }

  onJoinError(callback, context){
    !this.isonJoinError && this.events.on('join-error',callback, context)
    this.isonJoinError = true
  }

  onJoinSuccess(callback, context){
    !this.isOnJoinSuccess && this.events.on('join-success',callback, context)
